...
public class DBUtil {
    ...
    static {
        try {
            ...
            //用一个for循环来创建二十个连接
            for (int i = 0; i < max; i++) {
                con = DriverManager.getConnection("jdbc:mysql://localhost:3306/tasksystem?useUnicode=true&characterEncoding=UTF-8", "root", "123456");//这里的改编码方式很重要啊老表   对不起老表，好像没有这个我也操作的喔
     ...
     * 此处实现把连接释放
     * 说白了就是把连接加到链表去
     * 在finally语句中直接调用此方法，释放当时获取的连接，不需要close了
